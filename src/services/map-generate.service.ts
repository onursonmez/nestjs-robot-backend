...
@Injectable()
export class MapGenerateService {
    generateSvg(activeMap: Map): string {
        if (!activeMap) return '';
    ...
    `;
        ...
        return this.optimizeSvg(svgContent);
    }
    ...
    private optimizeSvg(svgContent: string): string {
        ...
        const optimizedRects = Object.keys(groupedRects).flatMap((fill) => {
            ...
            const mergedRects = this.mergeRects(rectsWithSameColor);
    ...
    private mergeRects(rects: Element[]): Rect[] {
        const mergedRects: Rect[] = [];
        let currentRect: Rect | null = null;

        rects.sort((a, b) => {
            const yDiff =
                parseInt(a.getAttribute('y') || '0') -
                parseInt(b.getAttribute('y') || '0');
